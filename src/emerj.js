/*
 * Emerj is a tiny JavaScript library to render live HTML/DOM updates
 * efficiently and non-destructively, by merging an updated DOM with
 * the live DOM, and only changing those elements that differ.
 *
 * https://github.com/bryhoyt/emerj/blob/f427476ec/src/emerj.js
 */

import { getAttrs } from "./utils.js";

export function nodesByKey(parent) {
  const map = {};

  for (let j = 0; j < parent.childNodes.length; j++) {
    const key = parent.childNodes[j].id;

    if (key) {
      map[key] = parent.childNodes[j];
    }
  }
...
export default function merge(base, modified) {
  ...
  for (idx = 0; modified.firstChild; idx++) {
    ...
    // the new order.
    const newKey = newNode.id;

    if (baseNode.id || newKey) {
      ...
      const match =
        newKey && newKey in allNodesByKey.old
          ? allNodesByKey.old[newKey]
          : newNode;
    ...
    if (
      ...
      if (baseNode.textContent === newNode.textContent) {
        continue; // Don't write if we don't need to.
      }
      ...
      const allAttrs = { base: getAttrs(baseNode), new: getAttrs(newNode) };

      for (const attr in allAttrs.base) {
        // Remove any missing attributes.
        if (attr in allAttrs.new) continue;
        baseNode.removeAttribute(attr);
      }

      for (const attr in allAttrs.new) {
        // Add and update any new or modified attributes.
        if (attr in allAttrs.base && allAttrs.base[attr] === allAttrs.new[attr])
          continue;
        baseNode.setAttribute(attr, allAttrs.new[attr]);
      }
