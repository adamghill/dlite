...
test("r-if", () => {
  const root = createHTML(
    `<body><div id='root'><span :if='this.a === b'>Hello</span></div></body>`
  );
...
test("greater & lesser than", () => {
  ...
  const rootB = createHTML(
    `<body><div id='root'><span :if='this.a < b'>Hello</span></div></body>`
  );
  const rootC = createHTML(
    `<body><div id='root'><span :if='this.a >= b'>Hello</span></div></body>`
  );
  const rootD = createHTML(
    `<body><div id='root'><span :if='this.a <= b'>Hello</span></div></body>`
  );
  parseDirectives(root);
  ...
  expect(rootC.innerHTML).toBe(
    '<div id="root">${this.a &gt;= b ? `<span>Hello</span>`:``}</div>'
  );
  expect(rootD.innerHTML).toBe(
    '<div id="root">${this.a &lt;= b ? `<span>Hello</span>`:``}</div>'
  );
});
...
test("r-if else", () => {
  ...
  parseDirectives(root);
  const result =
    '<div id="root">${this.a === b ? `<span>Hello</span>`:`<span>World</span>`}</div>';
  expect(root.innerHTML).toBe(result);
});
...
test("for", () => {
  const root = createHTML(
    `<body><div id='root'><ul><li :for="item in this.items">{item}</li></ul></div></body>`
  );
  parseDirectives(root);
  const result =
